      ...
      .on("end", async () => {
        ...
        if (!fs.existsSync(outputFilePath)) {
          ...
          for (const map of rows) {
            ...
            if (responseJson === "Error in fetching data") continue;
            ...
            let firstRow = [`${travelsName}_${serviceId}`];
            outputData.push(firstRow);
  
            const mainTable = [];
            const headerRow = ["Seats", presentDateAndTime];
            mainTable.push(headerRow);
  
            for (let i = 0; i < allSeatsList.length; i++) {
              const seat = allSeatsList[i];
              const value = seatMappings[seat].Is_Available
                ? seatMappings[seat].Fares_Amount
                : 0;
              mainTable.push([seat, value]);
            }
  
            console.log(mainTable);
  
            outputData.push(...mainTable);
  
            let lastRow = [`end of ${travelsName}_${serviceId}`];
            outputData.push(lastRow);
          }
  
          console.log("Writing output data to CSV file");
          const csvOutput = parse(outputData);
          ...
          fs.createReadStream(outputFilePath)
            ...
            .on("end", async () => {
              for (const map of rows) {
                ...
                const marker = `${travelsName}_${serviceId}`;
                const markerIndex = existingData.findIndex(row => row.Seat === marker);
                if (markerIndex === -1) continue;
  
                const lastColumn = Object.keys(existingData[markerIndex]).length;
                existingData[markerIndex][presentDateAndTime] = presentDateAndTime;
  
                for (let i = 0; i < allSeatsList.length; i++) {
                  const seat = allSeatsList[i];
                  const fare = seatMappings[seat].Is_Available ? seatMappings[seat].Fares_Amount : 0;
                  const seatIndex = existingData.findIndex(row => row.Seat === seat);
                  if (seatIndex !== -1) {
                    existingData[seatIndex][presentDateAndTime] = fare;
                  } else {
                    existingData.push({ Seat: seat, [presentDateAndTime]: fare });
                  }
                }
              }
  
              console.log("Appending new data to CSV file");
              const csvOutput = parse(existingData);
              fs.writeFileSync(outputFilePath, csvOutput);
